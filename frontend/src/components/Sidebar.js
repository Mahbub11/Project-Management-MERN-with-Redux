...
const Sidebar = () => {
  ...
  useEffect(() => {
    // projectData() // we can only call the function and add dependencies
    projectData()
  ...
  return (
    <div className='py-5'>
      <div className="px-4 mb-3 flex items-center justify-between">
        <span className='font-sans text-xl dark:text-white font-bold'>Projects</span>
      ...
      <ul className='border-r border-gray-300 pr-2'>
        {projects.map((project, index) => (
          <Link key={index} to={project._id} onClick={(e) => handleLocation(e)}>
            <li className={`px-5 h-10 py-1.5 mb-1 text-gray-600 dark:text-white font text-sm capitalize select-none hover:text-indigo-600 rounded transition-colors hover:bg-indigo-200/30 ${paramsWindow === project._id && 'text-indigo-600 bg-indigo-200/80 dark:bg-indigo-200/10 font-thin'}`}>
